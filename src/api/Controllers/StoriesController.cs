...
public class StoriesController(StoriesService storiesService) : ControllerBase
{
    ...
    [HttpGet("search")]
    public async Task<IActionResult> Search([FromQuery] string q, [FromQuery] int page = 1, [FromQuery] int pageSize = 30)
    {
        var result = await storiesService.SearchAsync(q, page, pageSize);
        Response.Headers["Cache-Control"] = "public, max-age=60";
        return Ok(result);
    }
}